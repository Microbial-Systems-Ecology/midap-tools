...
- Implemented `apply_offset_correction(offsets, by_group, frame_column)`: shifts frame indices forward for selected positions. Accepts a dict of `position → offset` (direct mode) or `group_value → offset` (by_group mode via metadata). Columns `frame`, `first_frame`, and `last_frame` are shifted; `split` (boolean flag) is left unchanged.
