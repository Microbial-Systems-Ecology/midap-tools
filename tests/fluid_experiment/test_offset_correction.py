...
def _make_experiment(positions, channels, frames_per_pos):
    ...
    frames_per_pos: dict of pos -> list of frame values to populate
    """
...
# ---------------------------------------------------------------------------
# Direct mode (offsets keyed by position name)
# ---------------------------------------------------------------------------

def test_direct_offset_shifts_frames(exp):
    exp.apply_offset_correction(offsets={"pos1": 3})
    assert list(exp.data["pos1"]["GFP"]["frame"]) == [3, 4, 5, 6, 7]


def test_direct_offset_shifts_first_last_frame(exp):
...
def test_direct_offset_split_unchanged(exp):
    exp.apply_offset_correction(offsets={"pos1": 3})
    assert list(exp.data["pos1"]["GFP"]["split"]) == [0, 0, 0, 0, 0]


def test_unspecified_position_unchanged(exp):
    original_frames = list(exp.data["pos2"]["GFP"]["frame"])
    exp.apply_offset_correction(offsets={"pos1": 3})
    assert list(exp.data["pos2"]["GFP"]["frame"]) == original_frames


def test_multiple_positions_offset(exp):
    exp.apply_offset_correction(offsets={"pos1": 2, "pos2": 5})
    assert list(exp.data["pos1"]["GFP"]["frame"]) == [2, 3, 4, 5, 6]
    assert list(exp.data["pos2"]["GFP"]["frame"]) == [5, 6, 7, 8, 9]
    assert list(exp.data["pos3"]["GFP"]["frame"]) == [0, 1, 2, 3, 4]  # untouched
...
# ---------------------------------------------------------------------------
# By-group mode (offsets keyed by metadata group value)
# ---------------------------------------------------------------------------
...
def test_by_group_shifts_matching_positions(exp_with_metadata):
    exp_with_metadata.apply_offset_correction(
        offsets={"Cond1": 5}, by_group="group"
    )
    assert list(exp_with_metadata.data["pos1"]["GFP"]["frame"]) == [5, 6, 7, 8, 9]
    assert list(exp_with_metadata.data["pos2"]["GFP"]["frame"]) == [5, 6, 7, 8, 9]


def test_by_group_unspecified_group_unchanged(exp_with_metadata):
    exp_with_metadata.apply_offset_correction(
        offsets={"Cond1": 5}, by_group="group"
    )
    assert list(exp_with_metadata.data["pos3"]["GFP"]["frame"]) == [0, 1, 2, 3, 4]
...
def test_by_group_multiple_groups(exp_with_metadata):
    ...
    assert list(exp_with_metadata.data["pos1"]["GFP"]["frame"]) == [3, 4, 5, 6, 7]
    assert list(exp_with_metadata.data["pos2"]["GFP"]["frame"]) == [3, 4, 5, 6, 7]
    assert list(exp_with_metadata.data["pos3"]["GFP"]["frame"]) == [7, 8, 9, 10, 11]
